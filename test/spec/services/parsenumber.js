...
describe('Service: parsenumber', function() {
  ...
  let parseNumber = {};
  let Game = {};
  beforeEach(inject(function(_parseNumber_, _Game_) {
  ...
  it('should do something', () => expect(!!parseNumber).toBe(true));

  it('parses numbers in different formats', function() {
    ...
    return expect(parseNumber('55e3000')+'').toBe('5.5e+3001');
  });

  it('parses number with fancy suffixes', function() {
    ...
    return expect(parseNumber('12qa').toNumber()).toBe(12e15);
  });

  it('parses cost-percents', function() {
    const game = mkgame({larva:'9e9999', meat:100});
    ...
    return expect(parseNumber('9999%', unit).toNumber()).toBe(10);
  }); //max

  it('parses twins', function() {
    const game = mkgame({larva:'9e9999', meat:'1e99999'});
    const unit = game.unit('drone');
    ...
    return expect(parseNumber('=1 thousand', unit).toNumber()).toBe(125);
  });

  it('parses a target value', function() {
    const game = mkgame({drone:0, larva:'9e9999', meat:'1e99999'});
    const unit = game.unit('drone');
    const upgrade = game.upgrade('dronetwin');
    expect(upgrade.count().toNumber()).toBe(0);
    expect(unit.twinMult().toNumber()).toBe(1);
    ...
    return expect(parseNumber('@1k', unit).toNumber()).toBe(100);
  });

  return it('parses =n%, #617', function() {
    const game = mkgame({drone:0, larva:'9e9999', meat:'1e99999'});
    const unit = game.unit('drone');
    return expect(parseNumber('=50%', unit).toNumber()).toBe(1);
  });
});
