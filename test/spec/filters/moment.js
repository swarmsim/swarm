...
describe('Filter: moment', function() {
  ...
  beforeEach(inject(function($filter, _options_) {
    duration = $filter('duration');
    options = _options_;
    // hack to make phantomJS work
    // https://stackoverflow.com/questions/2901102/how-to-print-a-number-with-commas-as-thousands-separators-in-javascript
    return Number.prototype.toLocaleString = function() {
      return this.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    };
  })
  );

  afterEach(() => Number.prototype.toLocaleString = toLocaleString);

  it('should format short durations', function() {
    const template = 'd[d] h:mm:ss';
    ...
    return expect(duration(123, 'days', template)).toBe('123d 0:00:00');
  });

  it('should format full durations', function() {
    ...
    return expect(duration(1234, 'days')).toBe('3 yr 4 mth 17 day 00:00:00');
  });

  it('should format abbreviated durations', function() {
    ...
    return expect(duration(1e21, 'years')).toBe('1.00000e21 years');
  });

  return it('should format human style durations', function() {
    ...
    return expect(duration(123, 'days')).toBe('4 months');
  });
});
