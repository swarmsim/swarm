...
angular.module('swarmApp').factory 'effecttypes', (EffectType, EffectTypes, util, seedrand, $log) ->
  ...
  effecttypes.register
    ...
    calcStats: (effect, stats, schema, level) ->
      ...
      stats[effect.stat] = (stats[effect.stat] ? new Decimal 1).times(Decimal.pow effect.val, level)
  effecttypes.register
    name: 'asympStat'
    calcStats: (effect, stats, schema, level) ->
      ...
      ONE = new Decimal 1
      stats[effect.stat] = (stats[effect.stat] ? ONE).plus new Decimal(effect.val).minus(1).times(ONE.minus(ONE.dividedBy(weight.plus(1))))
  effecttypes.register
    name: 'logStat'
    calcStats: (effect, stats, schema, level) ->
      ...
      stats[effect.stat] = (stats[effect.stat] ? ONE).times(effect.val3 ? 1).times(Decimal.log(level.times(effect.val).plus(effect.val2)).dividedBy(Decimal.log(effect.val2).minus(1))).plus(1)
  effecttypes.register
    name: 'addStat'
    calcStats: (effect, stats, schema, level) ->
      validateSchema effect.stat, schema, 'add'
      stats[effect.stat] = (stats[effect.stat] ? new Decimal 0).plus(level.times effect.val)
  # multStat by a constant, level independent
  effecttypes.register
    name: 'initStat'
    calcStats: (effect, stats, schema, level) ->
      validateSchema effect.stat, schema, 'mult'
      stats[effect.stat] = (stats[effect.stat] ? new Decimal 1).times(effect.val)
  effecttypes.register
    name: 'multStatPerAchievementPoint'
    calcStats: (effect, stats, schema, level) ->
      validateSchema effect.stat, schema, 'mult'
      points = effect.game.achievementPoints()
      stats[effect.stat] = (stats[effect.stat] ? new Decimal 1).times(Decimal.pow new Decimal(1).plus(effect.val).times(points), level)
  effecttypes.register
    name: 'suffix'
    calcStats: (effect, stats, schema, level) ->
